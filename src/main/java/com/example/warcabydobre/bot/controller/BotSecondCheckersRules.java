...
public class BotSecondCheckersRules implements GameRules {
	...
	/**
	 * Constructor of SecondCheckersRules class
	 *
	 * @param gameData model of the game
	 */
	public BotSecondCheckersRules(GameData gameData) {
	...
	/**
	 * Method checking if queen can move to passed field according to second
	 * checkers' rules.
	 * 
	...
	/**
	 * Method returning all possible queen piece's capture moves
	 * 
	 * @param piece passed piece object
	 * @return if piece is queen list of all possibles queen's capture moves
	...
	/**
	 * Method returning all possible queen piece's non-capture moves moves
	 * 
	 * @param piece passed piece object
	 * @return if piece is queen list of all possibles queen's non-capture moves
	 *         otherwise empty list
	 */
		...
		if (piece.isQueen()) {
			if (!canPlayerCapture(piece.getColor())) {
				...
				while (counter < 1 && newX >= 0 && newY >= 0) {
					if (!gameData.isOccupied(newX, newY)) {
						ModelMove move = new ModelMove(MovementTypes.QUEEN_DIAGONAL, oldX, oldY, newX, newY);
						posMoves.add(move);
					} else {
						counter++;
					}
	...
	/**
	 * method returning list of all possibles moves of passed piece
	 * 
	 * @param piece passed piece object
	...
	public LinkedList<ModelMove> possibleMoves(PieceObject piece) {
		...
		if (piece.isQueen()) {
			...
			if (canPlayerCapture(piece.getColor())) {
				...
				if (isCaptureMove(oldX, oldY, oldX - 2, oldY + 2 * stepY)) {
					PieceObject secondPiece = gameData.getPiecesArray()[oldX - 1][oldY + stepY];
	 ...
	 * 
	 * @param oldX the x coordinate of initial position of the piece
	 * @param oldY the y coordinate of initial position of the piece
	 * @param newX the x coordinate of final position of the piece
	...
	public boolean isCaptureMove(int oldX, int oldY, int newX, int newY) {
		...
		if (Math.abs(newX - oldX) != 2 || (newY - oldY) != 2 * pieceObject.getMovementDirection()) {
			return false;
		}
		...
		for (int y = 0; y < numRows; y++) {
			for (int x = 0; x < numCols; x++) {
				if (gameData.isOccupied(x, y)) {
					...
					if (currentPiece.getColor() == color && !possibleMoves(currentPiece).isEmpty()) {
						return true;
					}
				}
			}
	...
	public ModelMove tryMove(int oldX, int oldY, int newX, int newY) throws InvalidMoveException {
		...
		else if (Math.abs(newX - oldX) == 2 && (newY - oldY) == 2 * pieceObject.getMovementDirection()) {
			int x1 = oldX + (newX - oldX) / 2;
			int y1 = oldY + (newY - oldY) / 2;

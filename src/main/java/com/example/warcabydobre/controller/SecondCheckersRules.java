...
public class SecondCheckersRules implements GameRules {
	 ...
	 */
	public SecondCheckersRules(BoardModel boardModel) {
		...
		if (piece.isQueen()) {
			...
			if (canPlayerCapture(piece.getColor())) {
				...
				if (isCaptureMove(oldX, oldY, oldX - 2, oldY + 2*stepY)) {
					PieceObject secondPiece = boardModel.getPiecesArray()[oldX - 1][oldY + stepY];
